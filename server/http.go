...
func listenAndServe(addr string, mux *http.ServeMux, tlfConf *tls.Config, stop <-chan bool) error {
	...
	go func() {
		...
		if server.TLSConfig != nil {
			...
			if globals.configs.TlsRedirectHTTP != "" {
				log.Printf("Redirecting connections from HTTP at [%s] to HTTPS at [%s]",
					globals.configs.TlsRedirectHTTP, server.Addr)

				// This is a second HTTP server listenning on a different port.
				go http.ListenAndServe(globals.configs.TlsRedirectHTTP, tlsRedirect(server.Addr))
			}
...
func hstsHandler(handler http.Handler) http.Handler {
	if globals.configs.TlsStrictMaxAge != "" {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Strict-Transport-Security", "max-age="+globals.configs.TlsStrictMaxAge)
...
func (w *errorResponseWriter) Write(p []byte) (n int, err error) {
	if w.status >= http.StatusBadRequest {
		now := types.TimeNow()
		p, _ = json.Marshal(
			&DMClientMsg{Ack: &DMAckMsg{
				Timestamp: &now,
				IsOk:      false,
				Msg:       fmt.Sprintf("code=%v text:%s", w.status, http.StatusText(w.status))}})
	}
