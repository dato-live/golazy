...
const (
	...
	pongWait = types.DefaultIdleSessionTimeoutSecond
...
func (sess *Session) readLoop() {
	...
	sess.ws.SetReadLimit(globals.configs.MaxMessageSize)
...
func serveWebSocket(wrt http.ResponseWriter, req *http.Request) {
	...
	if isValid, _ := checkAPIKey(getAPIKey(req)); !isValid {
		...
		//TODO:状态信息应答需要完善
		json.NewEncoder(wrt).Encode(DMClientMsg{
			Ack:   &DMAckMsg{IsOk: false, Msg: "Missing, invalid or expired API key", MsgID: "", Timestamp: &now},
			MsgID: "",
		})
	...
	if req.Method != http.MethodGet {
		...
		//TODO:状态信息应答需要完善
		json.NewEncoder(wrt).Encode(DMClientMsg{
			Ack:   &DMAckMsg{IsOk: false, Msg: "Invalid HTTP Method", Timestamp: &now},
			MsgID: "",
		})
